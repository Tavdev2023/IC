...
def summary_text(result: AnalysisResult) -> str:
    ...
    return (
        ...
        f"Mín: {format_temperature(result.minimum)} | Máx: {format_temperature(result.maximum)} | "
        f"Média: {format_temperature(result.average)} | Mudanças bruscas: {len(result.sudden_changes)}"
    )
